...
chat_box_t *init_chat_box(void)
{
    ...
    if (chat_box->bg == NULL || chat_box->text == NULL ||
        ...
        return NULL;
    chat_box->msg[0] = '\0';
    sfText_setPosition(chat_box->text, (sfVector2f){500, 500});
    sfText_setFont(chat_box->text, chat_box->font);
    sfRectangleShape_setSize(chat_box->bg, (sfVector2f){200, 35});
    sfRectangleShape_setPosition(chat_box->bg, (sfVector2f){500, 500});
    sfRectangleShape_setFillColor(chat_box->bg, sfBlack);
    return chat_box;
}

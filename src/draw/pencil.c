...
void display_cursor(main_t *storage)
{
    ...
    if (storage->window.cursor == 2) {
        storage->window.eraser_pos.x = (sfMouse_getPositionRenderWindow(storage->window.window).x) - 0;
        storage->window.eraser_pos.y = (sfMouse_getPositionRenderWindow(storage->window.window).y) - 24;
